...
class Node:
    ...
    def set_address(self, *addresses):  # type: (*str) -> None
        """
        Args:
            *addresses (str): ip address(es) with mask in format 'x.x.x.x/x'

        Note:
            Values in 'addresses' will rewrite current values. For example,
            if there are addresses 192.168.0.10 and 192.168.0.11 at mgmt port
            sp-0 and only one address 192.168.0.20 was passed, result
            addresses list will contain single 192.168.0.20
        """

        body_nodes = {self.name: list()}
        for addr in addresses:
            ip, mask = get_ip_and_mask(addr)
            body_nodes[self.name].append({'ipaddress': ip, 'netmask': mask})

        other_nodes = self._get_other_nodes()
        body_nodes.update(other_nodes)
        ...
        self._client.post(
            ...
            body={
                'params': {
                    'nodes': body_nodes,
                    'failover': [{
                        'ipaddress': self.port.virtual_address.ip,
                        'netmask': self.port.virtual_address.mask,
                    }],
                    'gateway': self.port.gateway,
                    'mtu': self.port.mtu,
                }
            }
        )

    def _get_other_nodes(self):  # type: () -> Dict
        rv = {}
        problem_nodes = []

        another_nodes = [node for name, node in self.port.nodes.items()
                         if name != self.name]

        for node in another_nodes:
            addresses = []
            for addr in node.addresses:
                addresses.append(
                    {'ipaddress': addr.ip, 'netmask': addr.mask}
                )
            if len(addresses) < 1:
                problem_nodes.append(node.name)
            rv[node.name] = addresses

        if len(problem_nodes) > 0:
            is_plural = len(problem_nodes) > 1
            raise TatlinClientError(
                'Empty ip addresses on the {0} {1}. Each node '
                'must have at least one ip address'.format(
                    'nodes' if is_plural else 'node',
                    ', '.join(problem_nodes)
                )
            )
